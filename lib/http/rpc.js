...
const Amount = require('../nrg/amount');
...
RPC.prototype.init = function init() {
  ...
  this.add('getblockheight', this.getBlockHeight);
...
RPC.prototype.getInfo = async function getInfo(args, help) {
  ...
  return {
    ...
    keypoolsize: 0,
    unlocked_until: 0,
    paytxfee: Amount.nrg(this.network.feeRate, true),
    relayfee: Amount.nrg(this.network.minRelay, true),
    errors: ''
  };
};
...
RPC.prototype.getBlockHeight = async function getBlockHeight(args, help) {
  if (help || args.length !== 0)
...
RPC.prototype.getMempoolInfo = async function getMempoolInfo(args, help) {
  ...
  return {
    ...
    mempoolminfee: Amount.nrg(this.mempool.options.minRelay, true)
  };
};
...
RPC.prototype.getTXOut = async function getTXOut(args, help) {
  ...
  return {
    ...
    value: Amount.nrg(coin.value, true),
...
RPC.prototype.getTXOutSetInfo = async function getTXOutSetInfo(args, help) {
  ...
  return {
    ...
    bytes_serialized: 0,
    hash_serialized: 0,
    total_amount: Amount.nrg(this.chain.db.state.value, true)
  };
};
  ...
  if (!this.network.selfConnect) {
    if (this.pool.peers.size() === 0) {
      throw new RPCError(errs.CLIENT_NOT_CONNECTED,
        'Block Collider is not connected!');
    }

    if (!this.chain.synced) {
      throw new RPCError(errs.CLIENT_IN_INITIAL_DOWNLOAD,
        'Block Collider is downloading blocks...');
    }
  }
...
RPC.prototype.txToJSON = function txToJSON(tx, entry) {
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    vout.push({
      value: Amount.nrg(output.value, true),
      n: i,
...
RPC.prototype.entryToJSON = function entryToJSON(entry) {
  return {
    size: entry.size,
    fee: Amount.nrg(entry.deltaFee, true),
    modifiedfee: 0,
    time: entry.time,
    height: entry.height,
