/*!
 * rpc-wallet.js - bitcoind-compatible json rpc for bcoin.
...
RPC.prototype.dumpWallet = async function dumpWallet(args, help) {
  ...
  const out = [
    util.fmt('# Wallet Dump created by Bcoin %s', 1),
...
RPC.prototype._toWalletTX = async function _toWalletTX(wtx) {
  ...
  for (let i = 0; i < details.outputs.length; i++) {
    ...
    if (member.path) {
      ...
      det.push({
        ...
        category: 'receive',
        amount: Amount.nrg(member.value, true),
    ...
    det.push({
      ...
      category: 'send',
      amount: -(Amount.nrg(member.value, true)),
      fee: -(Amount.nrg(details.fee, true)),
      vout: i
    });
  ...
  return {
    amount: Amount.nrg(receive ? received : -sent, true),
...
RPC.prototype.getUnconfirmedBalance = async function getUnconfirmedBalance(args, help) {
  ...
  return Amount.nrg(balance.unconfirmed, true);
};
...
RPC.prototype.getWalletInfo = async function getWalletInfo(args, help) {
  ...
  return {
    walletid: wallet.id,
    walletversion: 6,
    balance: Amount.nrg(balance.unconfirmed, true),
    unconfirmed_balance: Amount.nrg(balance.unconfirmed, true),
    txcount: wallet.txdb.state.tx,
    keypoololdest: 0,
    keypoolsize: 0,
    unlocked_until: wallet.master.until,
    paytxfee: Amount.nrg(this.wdb.feeRate, true)
  };
};
...
RPC.prototype._listReceived = async function _listReceived(minconf, empty, watchOnly, account) {
  ...
  for (const entry of out) {
    ...
    entry.amount = Amount.nrg(entry.amount, true);
    result.push(entry);
  }
